...
df_src = spark.sql('''
select distinct(Date_ID) as Date_ID from parquet.`abfss://silver@prjtdelta.dfs.core.windows.net/carsales`
''')
df_src.display()
...
if spark.catalog.tableExists('deltaprjt.gold.dim_date'):
    df_sink= spark.sql('''
    select dim_date_key,Date_ID
    from deltaprjt.gold.dim_date
    ''')
...
if incremental_flag=='0':
    ...
    select max(dim_date_key) from deltaprjt.gold.dim_date
    ''')
...
if spark.catalog.tableExists('deltaprjt.gold.dim_date'):
    delta_tbl=DeltaTable.forPath(spark,"abfss://gold@prjtdelta.dfs.core.windows.net/dim_date")
    ...
    df_final.write.format('delta')\
        .mode('overwrite')\
        .option('path','abfss://gold@prjtdelta.dfs.core.windows.net/dim_date')\
        .saveAsTable('deltaprjt.gold.dim_date')


# COMMAND ----------

# MAGIC %sql
# MAGIC select * from deltaprjt.gold.dim_date